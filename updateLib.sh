#!/usr/bin/env bash

printf "Copying Files into Sub Directories..."
cp cvlib.py AMX/Crystal/cvlib.py
cp cvlib.py AMX/Crystal/pics/cvlib.py
cp cvlib.py AMX/Crystal/jeanImg/cvlib.py
cp cvlib.py AMX/RawImages/cvlib.py
cp cvlib.py AMX/etc/cvlib.py
cp cvlib.py CVLibrary/cvlib.py
cp cvlib.py EPICS/cvlib.py
cp cvlib.py IXS/cvlib.py
cp cvlib.py IXS/BPM/cvlib.py
cp cvlib.py IXS/HFM/cvlib.py
cp cvlib.py IXS/Merlin/cvlib.py
cp cvlib.py IXS/OnAxis2/cvlib.py
cp cvlib.py IXS/OnAxisImg/cvlib.py
cp cvlib.py IXS/VFM/cvlib.py
cp cvlib.py KazImages/Crystals/cvlib.py
cp cvlib.py KazImages/XrayImg/cvlib.py
cp cvlib.py Showcase/cvlib.py
cp cvlib.py Showcase/BenchTest/cvlib.py
cp cvlib.py tests/cvlib.py
cp cvlib.py XPD/cvlib.py
printf "\n"
printf "All Folders updated with new cvlib.py file..."
printf "\n"
